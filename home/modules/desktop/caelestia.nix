{
      ...
      settings = {
        theme = {
          ...
          postHook = ''
            ...
            gtk_css="$theme_dir/gtk-portal.css"
            legacy_gtk_css="$theme_dir/gtk.css"
            ...
            if [ -f "$gtk_css" ]; then
              ...
              cp "$gtk_css" "$data_home/themes/Caelestia-Portal/gtk-3.0/gtk.css"
              cp "$gtk_css" "$data_home/themes/Caelestia-Portal/gtk-4.0/gtk.css"

              for global_gtk in "$config_home/gtk-3.0/gtk.css" "$config_home/gtk-4.0/gtk.css"; do
                if [ -f "$global_gtk" ] && cmp -s "$gtk_css" "$global_gtk"; then
                  rm -f "$global_gtk"
                elif [ -f "$legacy_gtk_css" ] && [ -f "$global_gtk" ] && cmp -s "$legacy_gtk_css" "$global_gtk"; then
                  rm -f "$global_gtk"
                fi
              done
            fi
  ...
  home.activation.removeGlobalCaelestiaThemeCopies = lib.hm.dag.entryAfter ["writeBoundary"] ''
    ...
    if [ -f "$theme_dir/gtk-portal.css" ]; then
      ...
      for portal_gtk in "$data_home/themes/Caelestia-Portal/gtk-3.0/gtk.css" "$data_home/themes/Caelestia-Portal/gtk-4.0/gtk.css"; do
  ...
  xdg.configFile."caelestia/templates/gtk-portal.css".text = ''
    /*
      GTK portal file chooser only.
      Targeted surfaces: filechooser, pathbar, places sidebar, file views, and action bar.
    */
    ...
    @define-color portal_view #{{ surfaceContainerLowest.hex }};
    ...
    @define-color portal_panel_peak #{{ surfaceContainerHighest.hex }};
    @define-color portal_muted #{{ onSurfaceVariant.hex }};
    @define-color portal_border #{{ outlineVariant.hex }};
    @define-color portal_outline #{{ outline.hex }};
    @define-color portal_shadow #{{ shadow.hex }};

    filechooser {
      color: @portal_fg;
      background-color: @portal_bg;
      background-image: linear-gradient(135deg, alpha(@portal_secondary, 0.08), transparent 34%, alpha(@portal_tertiary, 0.07));
    }
    ...
    window.filechooser.csd,
    window.filechooser .csd {
      border-radius: 12px;
      box-shadow: 0 18px 48px alpha(@portal_shadow, 0.42);
    }

    window.filechooser headerbar,
    window.filechooser .titlebar {
      min-height: 44px;
      padding: 6px 10px;
      color: @portal_fg;
      background-color: @portal_bg;
      background-image: linear-gradient(to bottom, alpha(@portal_accent, 0.13), transparent 65%);
      border-bottom: 1px solid alpha(@portal_border, 0.78);
      box-shadow: inset 0 -2px alpha(@portal_accent, 0.42);
    }

    window.filechooser headerbar:backdrop,
    window.filechooser .titlebar:backdrop {
      color: @portal_muted;
    ...
    window.filechooser headerbar button,
    ...
    filechooser .path-bar button {
      ...
      padding: 5px 10px;
      color: @portal_fg;
      background-color: alpha(@portal_panel, 0.58);
      border: 1px solid alpha(@portal_border, 0.55);
      border-radius: 999px;
      box-shadow: none;
    }

    filechooser headerbar button image,
    window.filechooser headerbar button image,
    filechooser pathbar button image,
    filechooser .path-bar button image {
      -gtk-icon-size: 16px;
    }

    filechooser headerbar button:hover,
    window.filechooser headerbar button:hover,
    filechooser pathbar button:hover,
    filechooser .path-bar button:hover {
      color: @portal_fg;
      background-color: alpha(@portal_secondary, 0.18);
      border-color: alpha(@portal_secondary, 0.62);
    }

    filechooser headerbar button:checked,
    window.filechooser headerbar button:checked,
    ...
    filechooser placessidebar {
      ...
      background-image: linear-gradient(180deg, alpha(@portal_secondary, 0.10), transparent 42%, alpha(@portal_accent, 0.06));
    ...
    filechooser columnview > listview,
    ...
    filechooser viewport {
      ...
      background-image: linear-gradient(180deg, alpha(@portal_panel, 0.32), @portal_view 18%);
    }

    filechooser columnview header button,
    filechooser columnview button,
    ...
    filechooser columnview header button:hover,
    filechooser columnview button:hover,
    filechooser treeview.view header button:hover {
      color: @portal_fg;
      background-color: alpha(@portal_panel_high, 0.72);
      border-color: alpha(@portal_secondary, 0.46);
    }
    ...
    filechooser gridview > child:hover {
      background-color: alpha(@portal_tertiary, 0.10);
      background-image: linear-gradient(90deg, alpha(@portal_tertiary, 0.10), alpha(@portal_secondary, 0.05));
    }
    ...
    filechooser gridview > child:selected {
      color: @portal_fg;
      background-color: alpha(@portal_accent, 0.22);
      background-image: linear-gradient(90deg, alpha(@portal_accent, 0.24), alpha(@portal_secondary, 0.08));
      box-shadow: inset 3px 0 @portal_accent;
    }

    filechooser columnview row:selected *,
    filechooser columnview row:selected cell {
      color: @portal_fg;
      background-color: transparent;
      background-image: none;
      border-radius: 0;
    ...
    filechooser columnview row:selected filelistcell,
    filechooser listview row:selected filelistcell,
    filechooser gridview > child:selected filelistcell {
      color: @portal_fg;
    }

    filechooser filelistcell {
      min-height: 34px;
      color: inherit;
    }
